...
var Home = () => {
        ...
        if (imageData.length === 0) {
            ...
            var feedpics = imageData.map((datas) => {
                ...
                return (
                    <P_dummy
                        likebpic={likebutsrc}
                        commentbutid={"commentno" + p_c}
                        compg={(fid) => {
                            op_minpg(fid);
                        }}
                        txtuname={othuname}
                        txtcmnt={othcomm}
                        totcomms={"totcomms" + p_c}
                        countcomms={datas.comment_count}
                        totlikes={"totlikes" + p_c}
                        countlikes={datas.like_count}
                        likeid={"likeid" + p_c}
                        countcm={p_c}
                        commid={"commid" + p_c}
                        commnt_val={fetch_comment}
                        post_cmnt={() => {
                            post_cmnt(datas._id);
                        }}
                        like={(nid) => {
    ...
    var op_minpg = async (fid) => {
        minup(
            <div className="w-screen h-screen fixed pb-64 flex justify-center items-center">
        ...
        var comments = await Promise.all(
            extpicdata.comments.map(async (eachcom) => {
                ...
                return (
                    <div id="mincomdiv" key={com_map_key++}>
                        ...
                        <p id="mincomp1" key={com_map_key++}>
                            {eachcom.username}
                        </p>
                        <p id="mincomp2" key={com_map_key++}>
                            {eachcom.comment}
                        </p>
                    </div>
                );
        ...
        minup(
            <MinDisp
                minsendbut={() => {
                    sendcommin(extpicdata._id, picno);
                }}
                comments={comments}
                minpp={b64pic}
                minactpic={piclink}
    ...
    var sendcommin = async (sid, picno) => {
        ...
        if (inp) {
            ...
            document.getElementById("totcomms" + picno).innerHTML =
                Number(document.getElementById("totcomms" + picno).innerHTML) + 1;
        }
    };
    ...
    var post_cmnt = (id) => {
        ...
        if (message) {
            ...
            document.getElementById("totcomms" + postcount).innerHTML =
                Number(document.getElementById("totcomms" + postcount).innerHTML) + 1;
        }
    };

    var accpicextractor = async (usermail) => {
        profpicsup(
            <div className="w-[95%] mt-8 flex-wrap flex flex-row justify-center items-center justify-evenly">
                <HashLoader color="#FF1493" loading="true" size="100px" />
            </div>
        );
        await fetch(`${serverURL}/feed_pics/${usermail}`)
            ...
            .then((data) => {
                if (data.length < 1) {
                    profpicsup(
                        <h1 style={{ textAlign: "center" }}>NO IMAGES UPOADED</h1>
                    );
                } else {
                    var pic_files = data.map((datas) => {
                        ...
                        return (
                            <img
                                src={picsrc}
                                id="accimgs"
                                className=" w-32 h-32 md:w-64 md:h-64 mt-4"
                            />
                        );
                    });
                    profpicsup(
                        <div
                            id="profidcont"
                            className=" w-[95%] mt-4 flex-wrap flex flex-row justify-center items-start justify-evenly"
                        >
                            {pic_files}
                        </div>
                    );
    ...
    var editname = () => {
        ...
        if (newname.length !== 0) {
            ...
            errup(
                <p
                    id="errmsg"
                    style={{ color: "blue", fontSize: "16px", marginTop: "10px" }}
                >
                    Username Updated
                </p>
            );
    ...
    var editpass = async () => {
        ...
        if (oldpass.length === 0 || newpass.length === 0) {
            ...
            await fetch(`${serverURL}/editpass`, {
                method: "POST",
                body: JSON.stringify({
                    email: cookies.email,
                    oldpass: oldpass,
    ...
    var showfollowerlist = () => {
        ...
        fetch(`${serverURL}/fetchfollreq.body.oldpass ===  owdata`, {
            method: "POST",
            ...
            .then((data) => {
                var info = data.map((datas) => {
                    ...
                    return (
                        <div
                            id="foll_list"
                            className="flex flex-row mt-2 border-t-2 border-b-2 border-green-300 justify-center items-center"
                        >
                ...
                flistup(
                    <div
                        id="foll_box"
                        className="w-screen h-screen fixed flex flex-row justify-center items-center"
                    >
                        <div className="border-2 border-pink-400 w-full md:w-96 h-128 mt-[12rem] md:mt-16 bg-white flex flex-col">
                            <div
                                id="infoll_box"
                                className="w-full h-12 border-b-2 border-gray-300/80  flex flex-row justify-end items-center"
                            >
                                <p className="text-lg underline">Followers</p>
                                <input
                                    type="image"
                                    className="w-10 h-10 ml-[6.5rem]"
            ...
            .then((data) => {
                ...
                picshow(
                    <Profile
                        ...
                        name={data[0].name}
                        postno={data[0].postCount}
                        followerno={data[0].followerCount}
                        followingno={data[0].followingCount}
                        dp={linkimg}
                    />
                );
    ...
    async function search_peps_mob() {
        await fetch(
            `${serverURL}/search_people/` +
            document.getElementById("mob_search_bar").value.toLowerCase() +
            "/" +
            cookies.email
        )
            .then((res) => res.json())
                    ...
                    var userdat = data.map((users) => {
                        ...
                        if (users.email !== cookies.email) {
                            if (users.isFollowed === 0) {
                                return (
                                    <div
                                        className="w-full  h-12 mt-2 border-2 rounded-md border-lime-500 flex flex-row justify-center items-center justify-evenly"
                                        id={users.email}
                                    >
                                        <img
                                            className="w-8 h-8 rounded-2xl"
    ...
    var show_mob_searchbar = () => {
        picshow(
            <Search_box search_mob_inp={search_peps_mob} people={search_mob_ini} />
        );
    };
    ...
    async function followmech(event) {
        ...
        await fetch(
            `${serverURL}/followmech/` + peplist[sid - 1].email + "/" + cookies.email
        )
            .then(searchup())
    ...
    const list = (anchor) => (
        ...
        >
            ...
            <Button
                ...
                style={{ color: "#4BB543", marginTop: "1rem", borderRadius: "0px" }}
                color="secondary"
                variant="outlined"
            ...
            <Button
                className="w-full h-12"
                style={{ color: "red", marginTop: "1rem", borderRadius: "0px" }}
                color="secondary"
                variant="outlined"
    ...
    return (
        <div className="homepg">
            ...
            <React.Fragment key={"left"}>
                <nav
                    id="topnav"
                    className="w-full h-16 md:h-12 border-b-2 border-b-gray-200 flex flex-row fixed z-10 justify-around items-center bg-white"
                >
                    <button
                        className="block xl bg-yellow-400 md:hidden w-16 h-8 rounded-md"
                        onClick={toggleDrawer("left", true)}
                    >
                        OPEN
                    </button>
                    <input
                        className="hidden md:block border-2 h-6 border-pink-500 text-center border-solid rounded-none outline-none"
                        id="searchbar"
                        type="text"
